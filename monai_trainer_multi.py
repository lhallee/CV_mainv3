...
class Trainer(object):
    def __init__(self, config):
        ...
        train_sampler = DistributedSampler(train_ds, num_replicas=self.wold_size)
        valid_sampler = DistributedSampler(valid_ds, num_replicas=self.wold_size)
        self.train_loader = ThreadDataLoader(train_ds, num_workers=0, sampler=train_sampler,
                                             batch_size=config.batch_size, shuffle=True)
        self.valid_loader = ThreadDataLoader(valid_ds, num_workers=0, sampler=valid_sampler,
    ...
    def average_gradients(self, model):
        for param in model.parameters():
            ...
            param.grad.data /= self.wold_size
    ...
    def train(self):
        ...
        self.unet = DDP(self.unet, device_ids=[self.rank], output_device=self.rank)
        epoch = 0
        ...
        while epoch < self.num_epochs and self.best_unet_score < self.stop:
            ...
            for images, GT in self.train_loader:
                ...
                self.average_gradients(self.unet)
